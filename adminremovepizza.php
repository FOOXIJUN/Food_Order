...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['query'])) {
        ...
        $sql = "SELECT * FROM pizza WHERE name LIKE '%$searchQuery%' AND blocked = 0";
    ...
    <div style="text-align: right; padding: 20px;">
        <form method="GET" action="">
            <input type="text" name="query" placeholder="Search pizza by name" style="padding: 8px;">
    ...
    if (isset($_POST['block'])) {
        ...
        $query = "UPDATE pizza SET blocked = 1 WHERE id = $id";

        if (mysqli_query($conn, $query)) {
            echo "<script>alert('pizza has been blocked.');</script>";
        } else {
            echo "<script>alert('Error blocking pizza.');</script>";
        }
    }
    ...
    if (isset($_POST['unblock'])) {
        ...
        $query = "UPDATE pizza SET blocked = 0 WHERE id = $id";

        if (mysqli_query($conn, $query)) {
            echo "<script>alert('pizza has been unblocked.');</script>";
        } else {
            echo "<script>alert('Error unblocking pizza.');</script>";
        }
    }
    ...
    if (isset($_POST['remove'])) {
        ...
        if (mysqli_query($conn, $query)) {
            echo "<script>alert('pizza removed successfully.');</script>";
            echo '<meta http-equiv="refresh" content="0">';
        } else {
            echo "<script>alert('Error removing pizza.');</script>";
        }
    }
    ?>
