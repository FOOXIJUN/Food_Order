...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['query'])) {
        ...
        $sql = "SELECT * FROM burger WHERE name LIKE '%$searchQuery%' AND blocked = 0";
    ...
    <div style="text-align: right; padding: 20px;">
        <form method="GET" action="">
            <input type="text" name="query" placeholder="Search burger by name" style="padding: 8px;">
    ...
    if (isset($_POST['block'])) {
        ...
        $query = "UPDATE burger SET blocked = 1 WHERE id = $id";

        if (mysqli_query($conn, $query)) {
            echo "<script>alert('Burger has been blocked.');</script>";
        } else {
            echo "<script>alert('Error blocking burger.');</script>";
        }
    }
    ...
    if (isset($_POST['unblock'])) {
        ...
        $query = "UPDATE burger SET blocked = 0 WHERE id = $id";

        if (mysqli_query($conn, $query)) {
            echo "<script>alert('Burger has been unblocked.');</script>";
        } else {
            echo "<script>alert('Error unblocking burger.');</script>";
        }
    }
    ...
    if (isset($_POST['remove'])) {
        ...
        $query = "DELETE FROM burger WHERE id = '$id'";

        if (mysqli_query($conn, $query)) {
            echo "<script>alert('Burger removed successfully.');</script>";
            echo '<meta http-equiv="refresh" content="0">';
        } else {
            echo "<script>alert('Error removing burger.');</script>";
        }
    }
    ?>
